...
CMAP = {
    ...
    'ھ': 'ह',    # U+06BE  do-chashmi-he (standalone; aspirated pairs handled above)
...
# are never affected.  Ordered longest-first for safety.
# ─────────────────────────────────────────────────────────────────────────────
WORD_SUBS = [
    # ── Allah ─────────────────────────────────────────────────────────────
    ...
    # ── 7-char ────────────────────────────────────────────────────────────
    ('آنکھوں',    'आँखों'),       # āṃkhõ – eyes (obl.pl.)        (31x)
    ('آنکھیں',    'आँखें'),       # āṃkhẽ – eyes (nom.pl.)        (39x)

    # ── 6-char ────────────────────────────────────────────────────────────
    ('تمہارے',    'तुम्हारे'),    # tumhāre – your (m.pl.)         (755x)
    ('تمہاری',    'तुम्हारी'),    # tumhārī – your (f.)            (243x)
    ('دیکھتے',    'देखते'),       # dekhte – seeing/watching       (67x)
    ('دیکھیں',    'देखें'),       # dekhẽ – (they) see             (26x)
    ...
    ('انہوں',     'उन्हों'),      # unhõ – they (subj. pl.)       (313x)
    ('شیطان',     'शैतान'),       # shaitān – Satan/Devil          (103x)
    ('دیکھا',     'देखा'),        # dekhā – (he/she) saw           (107x)
    ('دیکھو',     'देखो'),        # dekho – look!/see!             (129x)
    ('آنکھ',      'आँख'),         # āṃkh – eye (sg.)              (87x)
    ('لیکن',      'लेकिन'),       # lekin – but/however            (143x)
    ('بلکہ',      'बल्कि'),       # balke – rather/instead         (149x)
    ('تاکہ',      'ताकि'),        # tāke – so that/in order to     (294x)
    ('پیدا',      'पैदा'),        # paidā – born/created           (323x)
    ('آئے',       'आए'),          # ā'e – came (m.pl.)            (197x)
    ('آئی',       'आई'),          # ā'ī  – came (f.)              (86x)
    ('گئے',       'गए'),          # ga'e – went (m.pl.)           (254x)
    ('گئی',       'गई'),          # ga'ī – went (f.)              (137x)
    ...
    ('دیکھ',      'देख'),         # dekh   – (look/see, verb stem) (117x standalone)
    ('کوئی',      'कोई'),         # ko'ī   – someone/anyone       (519x)
    ('کچھ',       'कुछ'),         # kuch   – something/some        (750x)
    ('پہلے',      'पहले'),        # pahle  – before/first          (302x)
    ('جہاں',      'जहाँ'),        # jahāṃ – where                  (49x)
    ('کہاں',      'कहाँ'),        # kahāṃ – where (question)       (33x)
    ('ضرور',      'ज़रूर'),       # zarūr  – certainly/must        (67x)
    ('صرف',       'सिर्फ़'),      # sirf   – only/just             (81x)
    ('بہت',       'बहुत'),        # bahut  – very/many             (408x)

    # ── Additional 4-char entries ─────────────────────────────────────────
    ('تیرا',      'तेरा'),        # terā – your (m.sg., infml.)    (frequent)
    ('تیرے',      'तेरे'),        # tere – your (m.pl., infml.)    (frequent)
    ('تیری',      'तेरी'),        # terī – your (f., infml.)       (already handled)
    ('میرا',      'मेरा'),        # merā – my (m.sg.)              (128x)
    ('میرے',      'मेरे'),        # mere – my (m.pl.)              (237x)
    ('میری',      'मेरी'),        # merī – my (f.)                 (106x)
    ('اوپر',      'ऊपर'),         # ūpar – above/on top            (37x)
    ('ایسا',      'ऐसा'),         # aisā – such/like this (m.sg.)  (149x)
    ('ایسے',      'ऐसे'),         # aise – such (m.pl.)            (217x)
    ('ایسی',      'ऐसी'),         # aisī – such (f.)               (107x)
    ('جیسا',      'जैसा'),        # jaisā – like/as (m.sg.)        (37x)
    ('جیسے',      'जैसे'),        # jaise – like/as (m.pl.)        (63x)
    ('جیسی',      'जैसी'),        # jaisī – like/as (f.)
    ('کیسا',      'कैसा'),        # kaisā – how/what kind (m.sg.)  (41x)
    ('کیسے',      'कैसे'),        # kaise – how/what kind (m.pl.)
    ('ویسا',      'वैसा'),        # vaisā – like that (m.sg.)      (16x)
    ('ویسے',      'वैसे'),        # vaise – like that (m.pl.)
    ('بھائی',     'भाई'),         # bhā'ī – brother                (81x)
    ('وہاں',      'वहाँ'),        # vahāṃ – there                  (93x)
    ('یہاں',      'यहाँ'),        # yahāṃ – here                   (118x)
    ('اُس',       'उस'),          # us    – that/it/him (obl. sg.) (53x)
    ...
    ('لیا',       'लिया'),        # liyā – took
    ('دیا',       'दिया'),        # diyā – gave
    ('گیا',       'गया'),         # gayā – went (m.sg.)
    ('ہوا',       'हुआ'),         # huā  – happened (m.sg.)        (252x)
    ('تجھ',       'तुझ'),         # tujh – you (obj.infml.)        (53x)
    ('تم',        'तुम'),         # tum   – you (pl./hon.)         (2908x)
    ('اس',        'उस'),          # is/us – this/that/it (obl.)    (3443x)
    ('ایک',       'एक'),          # ek    – one                    (654x)
    ('اسے',       'उसे'),         # use  – him/it/her              (268x)
    ('خدا',       'ख़ुदा'),       # Khudā – God                    (3175x)
    ('موت',       'मौत'),         # maut  – death                  (69x)
    ('مجھ',       'मुझ'),         # mujh  – me (direct)            (514x)
    ('بھی',       'भी'),          # bhī  – also/too/even           (1473x)
    ('تھے',       'थे'),          # the   – were (m.pl.)           (757x)
    ('تھا',       'था'),          # thā   – was (m.sg.)            (428x)
    ('تھی',       'थी'),          # thī   – was (f.)               (225x)
    ('جس',        'जिस'),         # jis   – which/who (oblique)    (871x)
    ('پھر',       'फिर'),         # phir  – then/again             (700x)
    ('ان',        'उन'),          # un    – those/them             (3914x)
    ('کہ',        'कि'),          # ke/ki – that/because           (3537x)
    ('نہ',        'न'),           # na    – not                    (1546x)
    ('بعد',       'बाद'),         # bād   – after                  (189x)
    ('دن',        'दिन'),         # din   – day                    (417x standalone)
