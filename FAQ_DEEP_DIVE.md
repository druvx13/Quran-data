...
**A:** Search is entirely client-side. When the search page opens it immediately fetches `sd/meta.json` (~200 KB) — a surah/ayah index. When you run a search, `search.html` reads your settings from `localStorage['quran-cf']` to determine which translation fields you have enabled, then lazily fetches only the corresponding `sd/{fieldkey}.json` files via `Promise.all()`. Already-fetched files are cached in memory so subsequent searches are instant. Result links navigate directly to the exact verse via `#ayah-N` anchors.

### Q: Why is the search slow on first load?

**A:** Only `sd/meta.json` (~200 KB) is fetched on page open. The per-field translation files are fetched on the first search. A user with the default fields enabled fetches roughly 3 MB total across those field files. On subsequent searches the cached data is reused. This is dramatically faster than the former approach of loading a single 27 MB `search-data.js` file on every visit.
...
**A:** The **static website** (GitHub Pages) does not collect any user data, set cookies, or run analytics. The only external connections are to Google Fonts (which may log requests per Google's privacy policy) and the Hugging Face audio server. The `localStorage` usage stores only local-device data — translation preferences (`quran-cf`), last-read surah (`quran-history`), and saved bookmarks (`quran-bookmark`) — and no data leaves the browser. For a self-hosted deployment, no additional GDPR compliance measures are required beyond the hosting provider's own policies.
