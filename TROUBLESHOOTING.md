...
**Causes & Fixes:**
1. **`search-data.js` is missing or empty.** This file is generated by `gendocshtml.py`. Regenerate it:
   ```bash
   python3 src/gendocshtml.py
   ```

2. **Browser blocked the JS file.** If viewing via `file://` URL, browsers may block local script loading. Use a local HTTP server:
   ```bash
   cd docs && python3 -m http.server 8000
   ```

3. **`search-data.js` loaded from cache.** Hard refresh: `Ctrl+Shift+R` (Windows/Linux) or `Cmd+Shift+R` (macOS).
