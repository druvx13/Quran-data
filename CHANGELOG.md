...
### Added
- **Hilali & Khan English PDF** (`output/hilali.pdf`, 644 pages) — Arabic text side-by-side
  with the English translation by Dr. Muhammad Taqi-ud-Din Al-Hilali &
  Dr. Muhammad Muhsin Khan, typeset with XeLaTeX.
  - New intermediate content file `latex/quhilali.tex` generated by `src/gentexforquran.py`
    from `data/en.hilali.txt` (`sura|ayah|text` format).
  - New LaTeX wrapper `latex/hilali.tex`.
  - `Makefile` updated: `output/hilali.pdf` added to the `all` target.
